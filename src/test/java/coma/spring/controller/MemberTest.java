...
public class MemberTest {
   private static final Logger logger = LoggerFactory.getLogger(MemberTest.class); // 어느 클래스에서 테스트되는 기록 용도
   @Autowired
   ...
   public void searchTest() {
      try {
         mockMvc.perform(MockMvcRequestBuilders.post("/map/search").
               param("keyword","와플대학"))
         .andDo(MockMvcResultHandlers.print())
         .andExpect(MockMvcResultMatchers.status().isOk()); 
   ...
   public void searchByCafeTest() {
      try {
         mockMvc.perform(MockMvcRequestBuilders.post("/map/searchCafeBtn"))
         .andDo(MockMvcResultHandlers.print())
         .andExpect(MockMvcResultMatchers.status().isOk());
   ...
   public void searchByFoodTest() {
      try {
         mockMvc.perform(MockMvcRequestBuilders.post("/map/searchFoodBtn"))
         .andDo(MockMvcResultHandlers.print())
         .andExpect(MockMvcResultMatchers.status().isOk());
   ...
   public void cafeInsertTest() {
      try {
         mockMvc.perform(MockMvcRequestBuilders.post("/map/cafeInsert").
               param("lng","").
               param("lat",""))
         .andDo(MockMvcResultHandlers.print())
         .andExpect(MockMvcResultMatchers.status().isOk());
   ...
   public void foodInsertTest() {
      try {
         mockMvc.perform(MockMvcRequestBuilders.post("/map/foodInsert").
               param("lng","").
               param("lat",""))
         .andDo(MockMvcResultHandlers.print())
         .andExpect(MockMvcResultMatchers.status().isOk());
   ...
   @Test 
   public void selectMarkerInfoTest() {
      try {
         mockMvc.perform(MockMvcRequestBuilders.get("/map/selectMarkerInfo").
               param("place_id","2066600910"))
         .andDo(MockMvcResultHandlers.print())
         .andExpect(MockMvcResultMatchers.status().isOk());
         logger.info("마커 클릭했을 때 페이지 이동 Test 성공!");
      } catch (Exception e) {
         logger.error("마커 클릭했을 때 페이지 이동 Test 실패 : " + e.getMessage());
         e.printStackTrace();
      }
   }
}